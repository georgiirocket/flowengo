...
pub async fn get_app_state(app_handle: AppHandle) -> Result<model::UserData, String> {
    ...
    Ok(model::UserData::new(
        state.is_initialized,
        state.user_name.clone(),
        state.create_date.clone(),
    ))
}
...
pub async fn sign_up(
    ...
    name: String,
    password: String,
) -> Result<model::SignUpResponse, String> {
    let common_store = app_handle
        .store(constants::STORE_PATH_COMMON)
        .map_err(|e| e.to_string())?;
    let protected_store = app_handle
        .store(constants::STORE_PATH_PROTECTED)
        .map_err(|e| e.to_string())?;

    let secure = crypto::SecureData::encrypt(&password, "{}".to_string())
        .await
        .map_err(|_e| "Failed to encrypt password")?;
...
pub async fn sign_in(
    app_handle: AppHandle,
    password: String,
) -> Result<model::SignInResponse, String> {
    let protected_store = app_handle
        .store(constants::STORE_PATH_PROTECTED)
        .map_err(|e| e.to_string())?;

    let value = protected_store
        .get("data")
        .ok_or("Protected store is empty")?;

    let secure: crypto::SecureData =
        from_value(value).map_err(|_e| "Failed to parse protected store")?;
    crypto::SecureData::decrypt(
        &password,
        secure.get_salt(),
        secure.get_nonce(),
        secure.get_secure_field(),
    )
    .map_err(|_e| "Incorrect password")?;
    ...
    let user_data = model::UserData::new(
        state.is_initialized,
        state.user_name.clone(),
        state.create_date.clone(),
    );
...
pub async fn get_protected_data(
    app_handle: AppHandle,
) -> Result<model::ProtectedDataResponse, String> {
    let state = app_handle.state::<Mutex<model::AppState>>();
    let password = {
        let state = state.lock().map_err(|e| e.to_string())?;
        state.get_password()
    ...
    let secure: crypto::SecureData =
        from_value(value).map_err(|_e| "Failed to parse protected store")?;
    let json_str = crypto::SecureData::decrypt(
        &password,
        secure.get_salt(),
        secure.get_nonce(),
        secure.get_secure_field(),
...
#[tauri::command]
pub async fn update_protected_data(
    app_handle: AppHandle,
    json_str: String,
) -> Result<model::UpdateProtectedDataResponse, String> {
    let state = app_handle.state::<Mutex<model::AppState>>();
    let password = {
        let state = state.lock().map_err(|e| e.to_string())?;
        state.get_password()
    ...
    let protected_store = app_handle
        .store(constants::STORE_PATH_PROTECTED)
        .map_err(|e| e.to_string())?;
    let secure = crypto::SecureData::encrypt(&password, json_str)
        .await
        .map_err(|_e| "Failed to encrypt data")?;
